...
import { ldmConfiguration } from './ldmConfiguration';
...
export class ldmController implements vscode.WebviewViewProvider {


	public static readonly viewType = 'ldm.controller';
  public static view_object: ldmController;
  ...
  private static is_config_watcher_set: boolean = false;
    ...
    ldmController.view_object = this;

    ldmController.set_build_watcher();
  }
  ...
  public static set_build_watcher() {
    ...
    if (ldmController.is_config_watcher_set || !ldmTools.contains_ldm_project())
      return;
  ...
  public static async update(): Promise<void> {
    ...
    if (ldmController.view_object && ldmController.view_object._view) {
      ldmController.view_object.resolveWebviewView(
        ldmController.view_object._view,
        ldmController.view_object._context!,
        ldmController.view_object._token!
      );
    }
  ...
  public static check_ldm_response() {

    const ws = Workspace.get_workspace();
    const ldm_status_file = path.join(ws, Constants.ldm_STATUS_FILE);

    if (DirTool.file_exists(ldm_status_file)) {
      const status = DirTool.get_json(ldm_status_file);

      if (status) {

        logger.info(`Status: ${JSON.stringify(status)}`);

        if (!status['version'] || status['version'] < Constants.ldm_BACKEND_VERSION) {
          vscode.window.showWarningMessage(
            'An update for ldm backend is available.',
            'Update'
          ...
          if (selection === 'Update') {
            ...
            const cmd = 'git pull';
            try {
              await SystemCmdExecution.run_system_cmd(config.general['local-ldm-dir'], cmd, 'update_ldm');
              vscode.window.showInformationMessage('ldm backend updated successfully.');
            } catch (error: any) {
  ...
  public static run_finished() {
    ldmController.view_object._view?.webview.postMessage({command: 'run_finished'});
    ldmController.check_ldm_response();
    ldmController.update_build_summary_timestamp();
    BuildSummary.update();
  ...
  public static update_build_summary_timestamp() {
    ...
    const compile_list = ldmTools.get_compile_list(rootPath);

    ldmController.view_object._view?.webview.postMessage(
      {
  ...
  public static async update_current_profile() {
    ...
    const compile_list: {}|undefined = ldmTools.get_compile_list(workspaceFolder);
    ...
    // Listener
        ...
        case 'run_build': // command:ldm.run_build
          ldmController.current_run_type = data.command;
          vscode.commands.executeCommand('ldm.run_build');
          break;

        case 'run_single_build': // command:ldm.run_build
          ldmController.current_run_type = data.command;
          vscode.commands.executeCommand('ldm.run_single_build');
          break;

        case 'show_changes':
          ldmController.current_run_type = data.command;
          vscode.commands.executeCommand('ldm.show_changes');
          break;

        case 'show_single_changes':
          ldmController.current_run_type = data.command;
          vscode.commands.executeCommand('ldm.show_single_changes');
          break;

        case 'cancel_running':
          SystemCmdExecution.abort_system_cmd('show_changes');
          SystemCmdExecution.abort_system_cmd('run_build');
          ldmTools.cancel('retrieve_current_source_hashes');
          break;

        case 'change_profile':
          AppConfig.change_current_profile(data.profile);
          ldmConfiguration.update();
          break;

        case 'copy_profile':
          vscode.commands.executeCommand('ldm.copy-profile-config');
          break;
  ...
  public static async copy_current_profile(): Promise<void> {
    ...
    let new_profile_config: string | undefined = await vscode.window.showInputBox({ title: `Copy profile config ${current_profile}`,
      ...
      value = Constants.ldm_APP_CONFIG_USER.replace('.toml', `-${value}.toml`);
    ...
    new_profile_config = Constants.ldm_APP_CONFIG_USER.replace('.toml', `-${new_profile_config}.toml`);
    const new_profile_config_file = path.join(Constants.ldm_CONFIGS_DIR, new_profile_config);
