...
import { ldmSourceConfiguration } from './ldmSourceConfiguration';
...
export class ldmConfiguration {

  public static currentPanel: ldmConfiguration | undefined;
  ...
  public static async render(context: vscode.ExtensionContext, extensionUri: Uri) {
    ...
    ldmConfiguration._context = context;
    ldmConfiguration._extensionUri = extensionUri;

    if (ldmConfiguration.currentPanel) {
      // If the webview panel already exists reveal it
      ldmConfiguration.currentPanel._panel.reveal(ViewColumn.One);
      return;
    }
    ...
    panel.webview.html = await ldmConfiguration.generate_html(context, extensionUri, panel.webview);

    panel.webview.onDidReceiveMessage(this.onReceiveMessage);

    ldmConfiguration.currentPanel = new ldmConfiguration(panel, extensionUri);
  ...
  private static async generate_html(context: vscode.ExtensionContext, extensionUri: Uri, webview: Webview): Promise<string> {
    ...
    const pwd = await context.secrets.get(`ldm|${host}|${user}`);
    ...
    const html = nunjucks.render('controller/configuration.html',
      {
        ...
        SSH_PASSWORD: pwd,
        project_config_file: DirTool.get_encoded_file_URI(Constants.ldm_APP_CONFIG_FILE),
        ...
        panel: await context.secrets.get('ldm|config|panel'),
        panel_tab: await context.secrets.get('ldm|config|panel_tab'),
  ...
  public static async update(): Promise<void> {
    ...
    const panel = ldmConfiguration.currentPanel;

    if (!panel)
      return;

    panel._panel.webview.html = await ldmConfiguration.generate_html(ldmConfiguration._context, ldmConfiguration._extensionUri, ldmConfiguration.currentPanel?._panel.webview);
  ...
  private static onReceiveMessage(message: any): void {
    ...
    if (message.panel)
      ldmConfiguration._context.secrets.store('ldm|config|panel', message.panel);
    if (message.panel_tab)
      ldmConfiguration._context.secrets.store('ldm|config|panel_tab', message.panel_tab);
    ...
    switch (command) {
      ...
      case "user_save":
        ldmConfiguration.save_config(is_user, workspaceUri, message.data);
        break;

      case "project_save":
        ldmConfiguration.save_config(is_project, workspaceUri, message.data);
        break;
      ...
      case "save_ssh_password":
        ...
        const host = config['connection']['remote-host'];
        const user = config['connection']['ssh-user'];
        ldmConfiguration._context.secrets.delete(`ldm|${host}|${user}`);
        if (message.password.length > 0)
          ldmConfiguration._context.secrets.store(`ldm|${host}|${user}`, message.password);
        break;
      ...
      case "add_language_attribute":
        ...
        ldmConfiguration.save_config(message.user_project == 'user', workspaceUri, config);
        break;
      ...
      case "edit_source_config":
        ...
        ldmSourceConfiguration.render(ldmConfiguration._context, ldmConfiguration._extensionUri, message.source);
        break;

      case "delete_source_config":
        ldmConfiguration.delete_source_config(message.source);
        ldmConfiguration.update();
        break;

      case "add_source_config":
        ldmConfiguration.add_source_config(message.source);
        ldmConfiguration.update();
        break;
  ...
  private static save_config(isUser: boolean, workspaceUri: Uri, data: {}) {
    ...
    // App config
    let toml_file = path.join(workspaceUri.fsPath, Constants.ldm_APP_CONFIG_FILE);
    if (isUser)
  ...
  private static createNewPanel(extensionUri : Uri) {
    return window.createWebviewPanel(
      'ldm_config', // Identifies the type of the webview. Used internally
      'ldm config', // Title of the panel displayed to the user
   ...
   */
  public dispose() {
    ldmConfiguration.currentPanel = undefined;
