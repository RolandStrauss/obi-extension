...
function main() {
    ...
    const run_build_button = document.getElementById("run_build");
    run_build_button?.addEventListener("click", run_build);
    const show_changes_button = document.getElementById("show_changes");
    show_changes_button?.addEventListener("click", show_changes);
    const cancel_running_button = document.getElementById("cancel_running");
    cancel_running_button?.addEventListener("click", cancel_running);
    const delete_current_profile_button = document.getElementById("btn_delete_current_profile");
    delete_current_profile_button?.addEventListener("click", delete_current_profile);
    const copy_profile_button = document.getElementById("btn_copy_profile");
    copy_profile_button?.addEventListener("click", copy_profile);
    const drp_use_profile = document.getElementById("drp_use_profile");
...
function get_run_type() {
    const run_type_drp = document.getElementById("opt_run_type");
    return run_type_drp.value;
}
function run_build() {
    let command = 'run_single_build';
...
function show_changes() {
    let command = 'show_single_changes';
