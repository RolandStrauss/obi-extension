...
class LogOutput {
     ...
     * @param extensionUri The URI of the directory containing the extension.
     */
    static render(workspaceUri, log_type, level, source, cmd_index) {
    ...
    static createNewPanel(log_type) {
        return vscode_1.window.createWebviewPanel('show_log', // Identifies the type of the webview. Used internally
    ...
    static get_log_content(workspaceUri, log_type, level, source, cmd_index) {
        const fs = require("fs");
        const config = AppConfig_1.AppConfig.get_app_config();
        let compile_list = BuildSummary_1.BuildSummary.get_compile_list();
